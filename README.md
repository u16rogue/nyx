...
* [nixos](./nixos) - NixOS configurations.
    ...
    * Systems/NixOS/Hosts are expected to use an ephemeral filesystem where each host and user are required to explicitly state which files and directories are to be preserved.
        * A `partial` preservation is available via `nyx.nixos.hosts.<hostname>.ephemeralfs.preserve.partial.directories` and the equivalent user option. Partial state is stored in `/persist/.partial/generations`, with `current` selecting the generation mounted this boot and `next` selecting an empty or previous generation to activate on the next boot.
        * `nyx ephemeralfs.partial new.boot` stages a fresh empty generation. Root-run `status`, `list`, `rollback`, `cancel`, and `delete` operations manage the lifecycle; switching only occurs during boot so active bind mounts never change underneath running programs.
        * `nyx.nixos.hosts.<hostname>.ephemeralfs.preserve.partial.root.enable` uses the current generation as a disk-backed writable root. The original tmpfs remains mounted underneath as a failure fallback. `/persist`, `/nix`, `/boot`, and explicitly preserved paths are mounted afterward and therefore override partial copies. Removing permanent preservation can reveal stale data in the partial generation; stage a fresh generation when changing that boundary unless the old data is intentionally wanted.
